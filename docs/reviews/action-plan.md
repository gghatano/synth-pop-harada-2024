...
> **ステータス（2026-05-04 時点）**: Phase 0〜3b の主要タスクは完了済み。本書は v0.x の設計判断・命名・優先順位の **経緯ドキュメント** として保持し、現在の到達点と残タスクは [`docs/status.md`](../status.md) を参照すること。本書自体は今後アクティブには更新しない（重要な方針変更は ADR で追補する）。
